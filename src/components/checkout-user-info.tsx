...
import { useCart, useUpdateCart } from '@modules/cart';
...
export function CheckoutUserInfo({ onDone, goBack }: CheckoutUserInfoProps) {
  ...
  const onSubmit = async (values: CheckoutUserInfoData) => {
    ...
    const { id, created_at, updated_at, customer_id, ...rest } =
  ...
  return (
    <Formik<CheckoutUserInfoData>
      ...
      validationSchema={toFormikValidationSchema<CheckoutUserInfoData>(
        validationSchema,
      )}
      onSubmit={onSubmit}>
